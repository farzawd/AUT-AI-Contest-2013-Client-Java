...
public class MyBot extends Bot
{
	...
	public void think()
	{
		...
		{
			if(getSuppliesAmount() >= 10 && getSelfAgents().getAllUnits().size() < 1)
		...
		for(Unit unit : units)
		{
			...
			// wall there) and no friendly unit is already there
			{
				...
				// check walkable
				if(getGameField().isWalkable(unit.getX() + dir.deltaX,
						unit.getY() + dir.deltaY))
					// check if there is no unit
					if(getSelfAgents().getUnitAt(unit.getX() + dir.deltaX,
							unit.getY() + dir.deltaY) == null)
						unit.move(dir);
			}
		}
		...
		ArrayList<Building.HeadQuarters> enemyBuildings = getEnemyAgents()
				.getHQsList();
